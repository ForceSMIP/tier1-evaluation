...
'Anchor-OPLS','UNet3D-LOCEAN','TrainingEM','RandomForest','EncoderDecoder','EnsFMP','ANN-Fingerprinters'};
    ...
    for j = 1:length(member)
        ...
        fields = fields_seasonal; clear fields_seasonal

        if strcmp(variable,'tos') || strcmp(variable,'zmta')
            % make sure NaNs in all the same land places
            mask = fields(:,:,:,2)./fields(:,:,:,2);
            mask_ref = field_ref./field_ref;
            mask_emean = field_emean./field_emean;
            field_ref = field_ref.*mask.*mask_ref.*mask_emean;
            fields = fields.*mask.*mask_ref.*mask_emean;
            field_emean = field_emean.*mask.*mask_ref.*mask_emean;
        end
        ...
        for i = 1:s(4)
            if strcmp(variable,'zmta')
                rmse_ref(j) = sqrt(global_mean(plev,lat,(mean((field_emean-field_ref).^2,3))'));
                rmse_all(i,j) = sqrt(global_mean(plev,lat,mean((field_emean-fields(:,:,:,i)).^2,3)'));
                std_emean(j) = sqrt(global_mean(plev,lat,mean(field_emean.^2,3)'));
                std_ref(j) = sqrt(global_mean(plev,lat,mean(field_ref.^2,3)'));
                std_all(i,j) = sqrt(global_mean(plev,lat,mean(fields(:,:,:,i).^2,3)'));
                %corr_ref(j) = global_mean(plev,lat,mean(field_emean.*field_ref,3)')./sqrt(global_mean(plev,lat,mean(field_emean.^2,3)').*global_mean(plev,lat,mean(field_ref.^2,3)'));
                %corr_all(i,j) = global_mean(plev,lat,mean(field_emean.*fields(:,:,:,i),3)')./sqrt(global_mean(plev,lat,mean(field_emean.^2,3)').*global_mean(plev,lat,mean(fields(:,:,:,i).^2,3)'));
                corr_ref(j) = global_mean(plev,lat,mean(field_emean.*field_ref,3)'./sqrt(mean(field_emean.^2,3)'.*mean(field_ref.^2,3)'));
                corr_all(i,j) = global_mean(plev,lat,mean(field_emean.*fields(:,:,:,i),3)'./sqrt(mean(field_emean.^2,3)'.*mean(fields(:,:,:,i).^2,3)'));
            else
                rmse_ref(j) = sqrt(global_mean(lon,lat,mean((field_emean-field_ref).^2,3)));
                rmse_all(i,j) = sqrt(global_mean(lon,lat,mean((field_emean-fields(:,:,:,i)).^2,3)));
                std_emean(j) = sqrt(global_mean(lon,lat,mean(field_emean.^2,3)));
                std_ref(j) = sqrt(global_mean(lon,lat,mean(field_ref.^2,3)));
                std_all(i,j) = sqrt(global_mean(lon,lat,mean(fields(:,:,:,i).^2,3)));
                %corr_ref(j) = global_mean(lon,lat,mean(field_emean.*field_ref,3))./sqrt(global_mean(lon,lat,mean(field_emean.^2,3)).*global_mean(lon,lat,mean(field_ref.^2,3)));
                %corr_all(i,j) = global_mean(lon,lat,mean(field_emean.*fields(:,:,:,i),3))./sqrt(global_mean(lon,lat,mean(field_emean.^2,3)).*global_mean(lon,lat,mean(fields(:,:,:,i).^2,3)));
                corr_ref(j) = global_mean(lon,lat,mean(field_emean.*field_ref,3)./sqrt(mean(field_emean.^2,3).*mean(field_ref.^2,3)));
                corr_all(i,j) = global_mean(lon,lat,mean(field_emean.*fields(:,:,:,i),3)./sqrt(mean(field_emean.^2,3).*mean(fields(:,:,:,i).^2,3)));
            end
        end
...
nmo = length(member);
nv = length(variables);
rmse_all_c = squeeze(sqrt(nanmean((rmse_all_all(:,:,:)./repmat(reshape(std_emean_all(:,:),[1 nmo nv]),[nme 1 1])).^2,2)));
rmse_ref_c = sqrt(nanmean((rmse_ref_all(:,:)./std_emean_all(:,:)).^2));
%corr_all_c = squeeze(sqrt(nanmean(corr_all_all(:,:,:).^2,2)));
%corr_ref_c = sqrt(nanmean(corr_ref_all(:,:).^2));
corr_all_c = squeeze(nanmean(corr_all_all(:,:,:),2));
corr_ref_c = nanmean(corr_ref_all(:,:));
...
to_plot(:,end+1) = nan;
to_plot(end+1,:) = nan;
plot_field_pixels(1:length(submission_short_names)+2,1:9,to_plot,linspace(-1,1,25))
...
%     to_plot(:,end+1) = nan;
%     to_plot(end+1,:) = nan;
%     plot_field_pixels(1:length(submission_short_names)+1,1:10,to_plot,linspace(-1,1,21))
%     set(gca,'color',[0.6 0.6 0.6])
%     pretty_figure(700,250,'none','none','none','none',16)
%     title([variable_labels{i},' MSSS Compared to Raw Data'],'fontsize',16)
%     set(gca,'ytick',1.5:1:9.5); set(gca,'yticklabel',models);
%     set(gca,'ydir','reverse'); set(gca,'box','off')
%     set(gca,'xtick',[1.5:1:10.5, 12.5:2:length(submission_short_names)+0.5]); set(gca,'xticklabel',[1:1:10, 12:2:length(submission_short_names)]);
%     xtickangle(0)
%     hold on; plot([1 31],[5 5],'k')
% 
%     to_plot = corr_all_all(:,is,i)-corr_ref_all(is,i)';
%     to_plot(:,end+1) = nan;
%     to_plot(end+1,:) = nan;
%     if sum(i == [1 2])==1
%         plot_field_pixels(1:length(submission_short_names)+1,1:10,to_plot,linspace(-0.1,0.1,21))
%     elseif sum(i == [3 4 5])==1
%         plot_field_pixels(1:length(submission_short_names)+1,1:10,to_plot,linspace(-0.4,0.4,21))
%     else
%         plot_field_pixels(1:length(submission_short_names)+1,1:10,to_plot,linspace(-0.2,0.2,21))
%     end
%     set(gca,'color',[0.6 0.6 0.6])
%     pretty_figure(700,250,'none','none','none','none',16)
%     title([variable_labels{i},' Pattern Correlation (Difference from Raw Data)'],'fontsize',16)
%     set(gca,'ytick',1.5:1:9.5); set(gca,'yticklabel',models);
%     set(gca,'ydir','reverse'); set(gca,'box','off')
%     set(gca,'xtick',[1.5:1:10.5, 12.5:2:length(submission_short_names)+0.5]); set(gca,'xticklabel',[1:1:10, 12:2:length(submission_short_names)]);
%     xtickangle(0)
